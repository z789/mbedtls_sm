...
static const size_t sm3_test_buflen[2] =
{
    3, 64
};
...
{
    int i, ret = 0;
    unsigned char sm3sum[SM3_DIGEST_SIZE];
    ...
    for (i = 0; i < 2; i++) {
        ...
        ret = mbedtls_sm3(sm3_test_buf[i], sm3_test_buflen[i], sm3sum);
        if (ret != 0) {
            goto fail;
        }

        if (memcmp(sm3sum, sm3_test_sum[i], sizeof(sm3sum)) != 0) {
            ret = 1;
            goto fail;
        }
